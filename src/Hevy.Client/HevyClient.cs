...
{
  private static readonly Uri ApiOrigin = new("https://api.hevyapp.com/", UriKind.Absolute);
  private readonly HttpClient httpClient;
  private readonly string apiKey;

  public HevyClient(HttpClient httpClient, HevyClientOptions options)
  {
    ArgumentNullException.ThrowIfNull(httpClient);
    ArgumentNullException.ThrowIfNull(options);

    httpClient.BaseAddress = ApiOrigin;
  ...
  public async Task<PagedResult<ExerciseHistoryEntry>> GetExerciseHistoryAsync(string exerciseTemplateId, int page, int pageSize, DateOnly? startDate, DateOnly? endDate, CancellationToken cancellationToken)
  {
    ...
    return new PagedResult<ExerciseHistoryEntry>(page, 1, response.ExerciseHistory);
  }
  ...
  private async Task<T> GetAsync<T>(string relativeUri, JsonTypeInfo<T> jsonTypeInfo, CancellationToken cancellationToken)
  {
    ...
    {
      throw new HevyException("transient_upstream", "The Hevy API is temporarily unavailable.", true, null, exception);
    }
  }
