...
internal static class MeasurementWriteTools
{
  ...
  internal static async Task<CallToolResult> CreateBodyMeasurement(IServiceProvider services, CreateBodyMeasurementCommand request, bool dry_run = false, CancellationToken cancellationToken = default)
  {
    var result = await new CreateBodyMeasurementUseCase(ToolResults.Client(services)).ExecuteAsync(request, dry_run, cancellationToken);
    if (dry_run) return ToolResults.Success(ToolResults.DryRunData<CreateBodyMeasurementRequest, BodyMeasurement>((CreateBodyMeasurementRequest)request), "Body-measurement payload is valid; no request was sent.", ToolResults.DryRunMeta());
  ...
  internal static async Task<CallToolResult> UpdateBodyMeasurement(
      IServiceProvider services,
      DateOnly date,
      UpdateBodyMeasurementCommand request,
      ...
      CancellationToken cancellationToken = default)
  {
    ...
    try
    {
      result = await new UpdateBodyMeasurementUseCase(ToolResults.Client(services)).ExecuteAsync(date, request, expected_updated_at, force, dry_run, cancellationToken);
    }
    ...
    if (dry_run) return ToolResults.Success(ToolResults.DryRunData<UpdateBodyMeasurementRequest, BodyMeasurement>((UpdateBodyMeasurementRequest)request), "Body-measurement replacement payload is valid; no request was sent.", dryRunMeta);
