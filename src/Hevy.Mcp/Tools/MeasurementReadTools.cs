...
internal static class MeasurementReadTools
{
  ...
  internal static async Task<CallToolResult> GetBodyMeasurements(IServiceProvider services, [Range(1, int.MaxValue)] int page = 1, [Range(1, 10)] int page_size = 10, [RegularExpression("^(compact|full)$")] string detail = "compact", CancellationToken cancellationToken = default)
  {
  ...
  internal static async Task<CallToolResult> GetBodyMeasurement(IServiceProvider services, DateOnly date, CancellationToken cancellationToken = default)
  {
