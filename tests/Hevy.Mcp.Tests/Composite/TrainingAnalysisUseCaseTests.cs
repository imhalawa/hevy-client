...
public sealed class TrainingAnalysisUseCaseTests
{
  ...
  public async Task VolumeExcludesSetsMissingWeightOrRepsAndOutputContainsNoCoachingLanguage()
  {
    ...
    var result = await new TrainingAnalysisUseCase(client, new FixedTimeProvider(Now)).SummarizeTrainingAsync(4, null, 100, null, default);
  ...
  {
    var service = new TrainingAnalysisUseCase(new FakeHevyClient(), new FixedTimeProvider(Now));
  ...
  {
    var service = new TrainingAnalysisUseCase(new FakeHevyClient(), new FixedTimeProvider(DateTimeOffset.Parse("2026-07-26T18:30:00+02:00")));
  ...
  public async Task DefaultRangeContinuationRemainsStableAfterCrossingMondayBoundary()
  {
    ...
    var service = new TrainingAnalysisUseCase(client, clock);
    var first = await service.GetWorkoutEvidenceAsync(4, null, 1, null, default);
  ...
  public async Task WeeklyBucketsUseExactHalfOpenUtcInstantsForNonMidnightRange()
  {
    ...
    var result = await new TrainingAnalysisUseCase(client, new FixedTimeProvider(Now)).SummarizeTrainingAsync(4, end, 100, null, default);
